...
func newLocal(id string) *Local {
	pwd, _ := os.Getwd()
	dotGitimpart := filepath.Join(pwd, ".gitimpart")
	if _, err := os.Stat(dotGitimpart); os.IsNotExist(err) {
		if err := os.MkdirAll(dotGitimpart, 0755); err != nil {
			panic(err)
		}
	}
	dir := filepath.Join(dotGitimpart, id)
...
func (f *Local) Transact(fn func(path string) (*RenderResult, error)) (*RenderResult, error) {
	r, err := fn(f.fs.Root())
	return r, err
}
