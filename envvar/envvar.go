...
const (
	// Prefix is the prefix of the environment variables used by gitimpart.
	// All the environment variables used by gitimpart start with this prefix.
	//
	...
	//
	// The environment variables for operational settings are used by gitimpart itself,
	...
	//
	// If the file is stored in the local git repository, gitimpart internally deduce the remote repository
	...
	// RawConfig contains the whole content of the gitimpart.yaml file.
	//
	// It is used to pass the content of the gitimpart.yaml file from the source repository
	// to the target repository when gitops with workflow_dispatch is used.
	//
	// When this environment variable is set, gitimpart does not read the gitimpart.yaml file.
	...
	//
	// gitimpart uses this environment variable to get the event payload.
	// gitimpart uses the event payload in two ways:
	// 1. To get the pull-request information like the pull-request number
	// 2. To get the content of the gitimpart.yaml file and the derived configuration when gitops with workflow_dispatch is used
	//
	// 1. is used when gitimpart is invoked by the pull_request event.
	// 2. is used when gitimpart is invoked on the target repository by the workflow_dispatch event.
	//    The workflow_dispatch is usually triggered by gitimpart ran by the pull_request event
